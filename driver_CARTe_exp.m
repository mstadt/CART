...
IC = [CARTe_PB0;CARTm_PB0;CARTe_T0;CARTm_T0;Cplx0;Tumor0;CARTe_T_exp0;B0];

%% set parameters
p = set_params('PKPDclin_with_biomarkers');
p.Tumor0 = Tumor0;
p.M0 = 12.1*2.5E9/0.117;
p.B0 = 0.5;
% change params here
%p.Rm = 2e-3;
p.Vb = 1; % normalize the compartment size...
p.Vt = 1; % normalize the compartment size...
...
[t_veh,y_veh] = ode15s(@(t,y) modeqns_PKPD_CARTe_sep(t,y,params),...
                                    tspan, IC, options);
fprintf('treatment simulation \n')
IC(1) = doseCART_tot; % add dose into CARTe_PB
[t_treat, y_treat] = ode15s(@(t,y) modeqns_PKPD_CARTe_sep(t,y,params),...
                                    tspan, IC, options);
